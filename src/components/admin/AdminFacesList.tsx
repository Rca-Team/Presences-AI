...
const AdminFacesList: React.FC<AdminFacesListProps> = ({ 
  ...
  const fetchTodayStatuses = useCallback(async (faceList: RegisteredFace[]) => {
    try {
      ...
      const [{ data: todayData }, { data: gateData }] = await Promise.all([
        supabase
      ...
      // Merge gate entries: present if not already present/late via attendance record
      (gateData || []).forEach(g => {
        const matchedEmpId = g.student_id && statusMap[g.student_id] !== undefined
          ? g.student_id
          : undefined;
        if (matchedEmpId && statusMap[matchedEmpId]?.status !== 'present' && statusMap[matchedEmpId]?.status !== 'late') {
          statusMap[matchedEmpId] = { status: 'present', time: format(new Date(g.entry_time), 'hh:mm a') };
        }
      });
          ...
          .map(record => {
            try {
              ...
              return {
                ...
                image_url: record.image_url || metadata.firebase_image_url || metadata.image || '',
                total_attendance: 0,
