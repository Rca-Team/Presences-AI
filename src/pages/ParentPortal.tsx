      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-4 py-5 pb-20">
        ...
        {child && (
          <>
              ...
              <CardContent className="p-5">
                ...
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                  <div className="rounded-lg border border-primary/30 bg-background/70 p-3">
                    <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Achievement Power</p>
                    <p className="mt-1 text-xl font-black text-primary">{Math.min(100, Math.round((summary?.attendanceRate ?? 0) * 0.7 + (summary?.badgeCount ?? 0) * 4))}%</p>
                  ...
                  <div className="rounded-lg border border-primary/30 bg-background/70 p-3">
                    ...
                    <p className="mt-1 text-xl font-black text-foreground">#{studentLeaderboard?.rank ?? '-'}</p>
                    <p className="text-xs text-muted-foreground">Class position right now</p>
                  </div>
                  <div className="rounded-lg border border-primary/30 bg-background/70 p-3">
                    <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Momentum</p>
                    <p className="mt-1 text-xl font-black text-foreground">{Math.max(1, Math.round((summary?.streak ?? 0) / 2 + 1))}x</p>
            ...
            <Card className="animate-enter border-primary/30 bg-gradient-to-br from-card via-card to-primary/5">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Sparkles className="h-4 w-4 text-primary" />
                  Enchanted Realtime Achievements
                </CardTitle>
              </CardHeader>
              <CardContent>
                {achievementFeed.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Realtime achievements will appear here as your child earns badges, rank boosts, and streak milestones.</p>
                ) : (
