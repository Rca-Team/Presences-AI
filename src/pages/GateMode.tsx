...
import {
  ...
  DoorOpen, ChevronUp, ChevronDown, AlertTriangle, CloudOff,
} from 'lucide-react';
  ...
  const startSession = useCallback(async (selectedGate: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('gate_sessions').insert({
        ...
        metadata:    { periodKey: activePeriodKey, class: className, section, subject },
      }).select('id').single();
      ...
      setEntries([]);
      setIsSetup(false);
      toast.success(`Gate Mode started — ${selectedGate}`);
    } catch (err: any) {
